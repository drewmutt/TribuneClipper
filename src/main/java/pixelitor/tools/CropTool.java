...
import com.aero.util.ImageUtil;
...
//    private JButton cropButton;
    ...
    CropTool() {
        super('c', "Crop", "crop_tool_icon.png",
                ...
                Cursor.getDefaultCursor(), false, false, true, ClipStrategy.IMAGE_ONLY);
    ...
    public void initSettingsPanel() {
        ...
        JButton extractButton = settingsPanel.addButton("Extract",
                e ->
                {
                    String title = "Pasted Image ";

                    Composition activeComp = ImageComponents.getActiveCompOrNull();
                    Rectangle2D cropRect = Tools.CROP.getCropRect(activeComp.getIC());
                    BufferedImage croppedImage = ImageUtil.cropImageToBounds(activeComp, cropRect.getBounds());
                    Composition comp = Composition.fromImage(croppedImage, null, title);
                    comp.setMetadata(activeComp.getMetadata().copy());
        ...
        JButton rotateButton = settingsPanel.addButton("Auto Rotate",
                e ->
                {
                    Composition activeComp = ImageComponents.getActiveCompOrNull();
                    BufferedImage compositeImage = activeComp.getCompositeImage();
                    double skewOfImage = ImageUtil.getSkewOfImage(compositeImage) ;
                    double degrees = skewOfImage * (180.0 / Math.PI);
                    System.out.println("Skew: " + degrees);

                    AffineTransform transform = new AffineTransform();
                    transform.rotate(skewOfImage, compositeImage.getWidth()/2, compositeImage.getHeight()/2);
                    AffineTransformOp op = new AffineTransformOp(transform, AffineTransformOp.TYPE_BILINEAR);
                    compositeImage = op.filter(compositeImage, null);

                    Composition comp = Composition.fromImage(compositeImage, null, "New thing");
                    comp.setMetadata(activeComp.getMetadata());
                    AppLogic.addComposition(comp);
                });


        JButton cropButton = settingsPanel.addButton("Crop",
                e ->
                {
                    ImageComponents.toolCropActiveImage(allowGrowingCB.isSelected());
                    ImageComponents.repaintActive();
        ...
        settingsPanel.add(extractButton);
        settingsPanel.add(rotateButton);
