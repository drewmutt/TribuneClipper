...
//        colorsMenu.buildFA("Levels 2", Levels2::new).add();
    ...
    private static JMenu createTribuneSubmenu()
    {
        PMenu tribuneMenu = new PMenu("Tribune");
        tribuneMenu.addAction(new FilterAction(AutoAdjust.NAME, AutoAdjust::new).withoutGUI());

        tribuneMenu.addAction(new MenuAction("Show Metadata/Upload...") {
            @Override
            public void onClick() {
                Composition comp = ImageComponents.getActiveCompOrNull();
                if(comp != null)
                {
                    CompositionMetadata metadata = comp.getMetadata();
                    if(metadata != null)
                    {
                        CompositionMetadata.showMetadataDialog(metadata);
                    }
                }
